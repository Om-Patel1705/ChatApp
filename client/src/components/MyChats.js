...
const MyChats = ({ fetchAgain }) => {
  ...
  const listUsers = async (chatID) => {
    try {
      const response = await axios.post('http://localhost:3001/listout', { chatID });
      startTransition(() => {
        setChatnames(response.data.chatnames);
      });
    } catch (error) {
  ...
  useEffect(() => {
    if (user) {
      listUsers(user.chatID); // Assuming chatID is available in the user object
    }
  }, [user]);
